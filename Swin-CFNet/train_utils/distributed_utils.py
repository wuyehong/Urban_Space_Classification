...
class ConfusionMatrix(object):
    ...
    def compute(self):
        ...
        total = h.sum()
        po = acc_global
        pe = (h.sum(0) * h.sum(1)).sum() / (total * total)
        kappa = (po - pe) / (1 - pe)
        mIoU = iu.mean().item()
        ...
        return acc_global.item() * 100, (acc * 100).tolist(), (iu * 100).tolist(), mIoU * 100, kappa * 100, (
                    f1_score * 100).tolist()
    ...
    def __str__(self):
        acc_global, acc, iu,mIoU,kappa,f1 = self.compute()
        return (
            'global correct: {:.1f}\n'
            'average row correct: {}\n'
            'IoU: {}\n'
